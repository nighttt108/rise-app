...
const QUEST_TYPE_COLORS = { normal: '#6b7280', hard: '#8b5cf6', raid: '#f59e0b', gate: '#ef4444' }
...
export function DashboardPage() {
  ...
  async function completeQuest(questId, baseXP) {
  ...
  function getDailyStreak(subPathId) {
    ...
    <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto', paddingBottom: '32px' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', overflowX: 'auto', paddingBottom: '4px' }}>
        {progress.map(p => {
          ...
          const { completed, total } = getDailyStreak(p.sub_path_id)
          const allDailyDone = total > 0 && completed === total

          return (
            <button
              key={p.genre_id}
              onClick={() => setExpandedGenre(isExpanded ? null : p.genre_id)}
              style={{
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                ...
                <div style={{
                  ...
                  fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 700, color
                }}>{p.current_rank}</div>
              </div>

              {/* Genre name */}
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: isExpanded ? color : 'var(--text-primary)', marginBottom: '6px' }}>
                {p.genres?.name}
              </div>
              ...
              <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                {Array.from({ length: total }).map((_, i) => (
                  <div key={i} style={{
                    ...
                    background: i < completed ? color : 'var(--border-dim)',
                    transition: 'background 0.2s'
                  }} />
                ))}
                {allDailyDone && <Zap size={10} color={color} style={{ marginLeft: '2px' }} />}
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                <div>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginTop: '2px' }}>
                    {p.total_xp.toLocaleString()} total XP
                  </div>
                </div>
                {nextRank && (
            ...
            {allDailyDone ? (
              <div style={{
                ...
                <span style={{ fontSize: '13px', fontWeight: 600, color, fontFamily: 'var(--font-display)' }}>
                  ALL DAILIES DONE — +50% BONUS ON WEEKLY XP
                </span>
              </div>
            ) : dailies.length > 0 && (
              <div style={{
                ...
                <span style={{ fontSize: '13px', color: '#f59e0b', fontFamily: 'var(--font-display)', fontWeight: 600 }}>
                  {completedDailies}/{dailies.length} DAILIES · Complete all 7 days for +50% weekly bonus
                </span>
              </div>
            )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
                  {dailies.map(q => (
                    <DailyQuestCard
                      ...
                      onComplete={() => completeQuest(q.id, 0)}
                  ...
                  {weeklies.map(q => {
                    ...
                    return (
                      <WeeklyQuestCard
                        ...
                        quest={q}
                        xp={finalXP}
                        bonusActive={allDailyDone}
                        completing={completing === q.id}
                        onComplete={() => completeQuest(q.id, finalXP)}
                        onUncomplete={() => uncompleteQuest(q.id)}
                        color={color}
                      />
                    )
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
                  {gates.map(q => (
                    <GateQuestCard
                      ...
                      quest={q}
                      completing={completing === q.id}
                      onComplete={() => completeQuest(q.id, q.quest_templates?.base_xp || 800)}
                    />
                  ))}
                </div>
              </>
...
function DailyQuestCard({ quest, completing, onComplete, onUncomplete }) {
  ...
  return (
    <div style={{
      background: done ? 'rgba(34,197,94,0.05)' : 'var(--bg-surface)',
      border: `1px solid ${done ? 'rgba(34,197,94,0.2)' : 'var(--border-dim)'}`,
      borderRadius: '12px', padding: '14px',
      display: 'flex', alignItems: 'flex-start', gap: '12px',
      transition: 'all 0.2s', opacity: done ? 0.7 : 1
    }}>
      <button onClick={done ? onUncomplete : onComplete} disabled={completing} style={{
        background: 'none', border: 'none', cursor: 'pointer',
        color: done ? '#22c55e' : 'var(--text-dim)',
        padding: '2px', flexShrink: 0, marginTop: '1px'
      }}>
        {completing
          ? <div style={{ width: '20px', height: '20px', border: '2px solid var(--text-dim)', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} />
          : done ? <CheckCircle2 size={20} /> : <Circle size={20} />
        }
      </button>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: '14px', fontWeight: done ? 400 : 500, color: done ? 'var(--text-dim)' : 'var(--text-primary)', textDecoration: done ? 'line-through' : 'none', marginBottom: '3px' }}>
          {quest.quest_templates?.title}
        </div>
        {!done && (
          <div style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
            {quest.quest_templates?.description}
          </div>
        )}
      </div>
...
function WeeklyQuestCard({ quest, xp, bonusActive, completing, onComplete, onUncomplete, color }) {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${done ? `${color}30` : 'var(--border-dim)'}`,
      borderRadius: '12px', padding: '14px',
      display: 'flex', alignItems: 'flex-start', gap: '12px',
      transition: 'all 0.2s'
    }}>
      <button onClick={done ? onUncomplete : onComplete} disabled={completing} style={{
        ...
        {completing
          ? <div style={{ width: '20px', height: '20px', border: `2px solid ${color}`, borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} />
          : done ? <CheckCircle2 size={20} /> : <Circle size={20} />
        }
      </button>
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: '14px', fontWeight: 500, color: done ? 'var(--text-secondary)' : 'var(--text-primary)', marginBottom: '4px', textDecoration: done ? 'line-through' : 'none' }}>
          {quest.quest_templates?.title}
        </div>
        {!done && (
          <div style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: done ? 'var(--text-dim)' : color }}>
          {done ? `+${quest.xp_awarded || xp}` : `+${xp}`} XP
        </div>
        {bonusActive && !done && (
          <div style={{ fontSize: '10px', color: '#f59e0b', textDecoration: 'line-through', marginTop: '1px' }}>
            {Math.round(xp / 1.5)}
          </div>
        )}
      </div>
...
function GateQuestCard({ quest, completing, onComplete }) {
  ...
  return (
    <div style={{
      background: done ? 'rgba(239,68,68,0.05)' : 'rgba(239,68,68,0.04)',
      border: `1px solid ${done ? 'rgba(239,68,68,0.4)' : 'rgba(239,68,68,0.25)'}`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
        ...
        <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: '#ef4444' }}>
          +{quest.quest_templates?.base_xp || 800} XP
        </span>
      </div>
      ...
      {!done ? (
        ...
        <div style={{ padding: '10px', background: 'rgba(239,68,68,0.1)', borderRadius: '8px', textAlign: 'center', color: '#ef4444', fontSize: '13px', fontWeight: 700, fontFamily: 'var(--font-display)' }}>
          ✓ GATE CLEARED
        </div>
      )}
    </div>
...
function SectionLabel({ children, color }) {
  return (
    <div style={{ fontFamily: 'var(--font-display)', fontSize: '11px', fontWeight: 600, color: color || 'var(--text-secondary)', letterSpacing: '0.15em', marginBottom: '8px' }}>
      {children}
    </div>
  )
}